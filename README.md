...
- validateDeliveryInfo - метод предназначен для валидации данных о способе оплаты и адресе получателя
- validateCustomerInfo - метод предназначен для валидации данных о номере телефона и адресе электронной почты получателя

### Класс ProductsModel
Предназначен для хранения каталога продуктов. 
Содержит следующие поля:
- products, типа IProducts[] - коллекция продуктов, полученная с сервера. При установке коллекции продуктов происходит генерация события products:loaded.
